...
stage.count %>%
  dplyr::filter(clinical_stage>0) -> cancers.clinicalStage

expr <- readr::read_rds(file.path(gsca_path,"expr","pancan33_expr.IdTrans.rds.gz"))

# combine data ------------------------------------------------------------
stage %>%
  dplyr::filter(cancer_types%in% cancers.clinicalStage$cancer_types) %>%
  tidyr::unnest() %>%
  .$clinical_stage %>%
  unique() %>% sort()
...
stage %>%
  ...
  dplyr::mutate(stage = purrr::map(stage,.f=function(.x){
    .x %>%
      ...
      dplyr::select(barcode,clinical_stage)
  })) %>%
...
stage_results.arrange %>%
  readr::write_rds(file.path(gsca_path,"expr","pan10_expr_with_ClinicalStage-210813.rds.gz"),compress = "gz")

save.image(file.path(git_path,"rda","gene_exp_with_ClinicalStage--210813.rda"))