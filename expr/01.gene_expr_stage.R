...
stage <- readr::read_rds(file.path(gsca_path,"clinical","pancan33_survival_age_stage_NEW.rds.gz")) %>%
  dplyr::mutate(stage=purrr::map(combine,.f=function(.x){
    .x %>%
