...
stage.count %>%
  dplyr::filter(igcccg_stage>0) -> cancers.igcccg_stage

expr <- readr::read_rds(file.path(gsca_path,"expr","pancan33_expr.IdTrans.rds.gz"))

# combine data ------------------------------------------------------------
stage %>%
  dplyr::filter(cancer_types%in% cancers.igcccg_stage$cancer_types) %>%
  tidyr::unnest() %>%
  .$igcccg_stage %>%
  unique() %>% sort()
stages_included <- tibble::tibble(stage=c("Stage I","Stage I","Stage I","Stage I","Stage I","Stage I","Stage I","Stage I",
                                  ...
                                  stage_raw=c("stage i","stage ia","stage ia1","stage ia2","stage ib","stage ib1","stage ib2","stage ic",
...
stage %>%
  ...
  dplyr::mutate(stage = purrr::map(stage,.f=function(.x){
    .x %>%
      dplyr::mutate(barcode=toupper(barcode),stage_type="igcccg_stage") %>%
      dplyr::select(barcode,stage=igcccg_stage,stage_type) %>%
      dplyr::filter(!is.na(stage))
  })) %>%
...
stage_results.arrange %>%
  readr::write_rds(file.path(gsca_path,"expr","TGCT_expr_with_igcccgStage-210813.rds.gz"),compress = "gz")

save.image(file.path(git_path,"rda","gene_exp_with_igcccgStage--210813.rda"))