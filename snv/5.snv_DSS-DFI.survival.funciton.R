...
fn_survival_res <- function(.cancer_types,.survival){
  ...
  if (length(grep("dss",colnames(.survival)))>0) {
    .combine %>%
      dplyr::mutate(survival_res = purrr::map(combine_data,fn_survival,sur_type="dss")) %>%
  ...
  if (length(grep("dfi",colnames(.survival)))>0) {
    .combine %>% 
      dplyr::mutate(survival_res = purrr::map(combine_data,fn_survival,sur_type="dfi")) %>%
      dplyr::select(-combine_data) %>%
      tidyr::unnest(cols = c(survival_res)) %>%
  ...
  tmp %>%
    readr::write_rds(file.path(res_path,"cancer_snv_DSS-DFI_survival_210914",paste(.cancer_types,"DSS-DFI_survival.snv.rds.gz",sep="_")),compress = "gz")
  tmp
}
